...
class Catalogo extends Component {
  ...
  componentDidMount() {
    axios
      .get('https://test-candidaturas-front-end.onrender.com/families?skip=0&take=10')
      .then(response => {
            ...
            <div className='w-full border-t-2 border-purple-300 shadow-md px-3 sm:px-0'>
                <div className='container py-7 mx-auto'>
                    ...
                    <div
                        ...
                        style={{ background: 'linear-gradient(90deg, rgba(161, 28, 243, 0.6) 0%, rgba(216, 53, 197, 0.6) 100%)' }}
                    ></div>
                    </h2>
                </div>
            </div>
                    ...
                    {cards.map(card => (
                        <div className='card sm:m-0' key={card.id}>
                            <div className='card-body border border-solid border-gray-300 rounded-md flex flex-col justify-center items-center bg-white'>
                                <div className='p-6'>
                                <Image src={`${imageBaseUrl}${card.id}${extension}`} alt='' width={200} height={200} className='h-40 sm:h-56 object-cover' />
                                </div>
                                <div className='w-full border-t-2 p-2'>
                                    <div className='w-full flex justify-between items-center'>
                                        <p className='font-sans text-black font-semibold text-xs mr-5 line-clamp-2'>{card.details.description}</p>
